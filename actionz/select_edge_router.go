...
func (self *SelectEdgeRouterAction) Execute(ctx *runzmd.ActionContext) error {
	for {
		...
		retry, err2 := runzmd.AskYesNoWithDefault(fmt.Sprintf("Error getting edge router (err=%v). Try again? [Y/N] (default Y): ", err), true)
		if err2 != nil {
...
func (self *SelectEdgeRouterAction) SelectEdgeRouter(ctx *runzmd.ActionContext) error {
	fmt.Println("")
